...
export function ReadinessKpiBand({ snapshot, onSelect }) {
  ...
  return (
    <section className="readiness-kpi-band hierarchy-secondary" aria-label="KPIs de prontidão e relacionamento da carteira">
      <ExecutiveSectionHeader
        eyebrow="Preparação"
        title="O que está pronto para executar?"
        note="5 indicadores de relacionamento"
        action={expanded ? 'Recolher indicadores' : 'Ver indicadores'}
        onAction={() => setExpanded(value => !value)}
        actionClassName="readiness-kpi-toggle"
      />
