...
const NAV_ITEMS = [
  { id: 'summary', label: 'Resumo executivo', short: 'Resumo', icon: LayoutDashboard, detail: 'decisÃ£o e risco' },
  { id: 'portfolio', label: 'Carteira', short: 'Carteira', icon: Briefcase, detail: 'capacidade e previsibilidade' },
  { id: 'demands', label: 'Demandas', short: 'Demandas', icon: ListChecks, detail: 'solicitaÃ§Ãµes Ã  agÃªncia' },
  { id: 'team', label: 'Time & performance', short: 'Time', icon: Users, detail: 'capacidade observÃ¡vel' },
  { id: 'analytics', label: 'Analytics Center', short: 'Analytics', icon: Activity, detail: 'volume, risco e fluxo' },
  { id: 'history', label: 'HistÃ³ria & Logs', short: 'HistÃ³ria', icon: Clock3, detail: 'evoluÃ§Ã£o e causas' }
];
...
export function ExecutiveDashboardShell({
  ...
  return (
    <div className="nexus-dashboard-shell">
      <aside className="nexus-sidebar" aria-label="NavegaÃ§Ã£o executiva">
        ...
        <div className={`nexus-sync-card ${syncTone}`}>
          ...
          <small>{version === null ? (consistencyMode === 'mixed' ? 'fontes mistas' : 'sem versÃ£o') : `${consistencyMode === 'mixed' ? 'espelho + fontes diretas' : 'espelho operacional'}`}</small>
        </div>
        <button type="button" className="nexus-sidebar-analyst" onClick={onOpenAnalyst}><Search size={15} /> Abrir analista <span>â†—</span></button>
      ...
      <main className="nexus-dashboard-main">
        <header className="nexus-topbar">
          ...
          <div className="nexus-topbar-actions">
            <span className="nexus-topbar-live"><Wifi size={13} /> {freshness === 'fresh' || freshness === 'live' ? 'Dados ao vivo' : freshness === 'stale' ? 'Leitura desatualizada' : freshness === 'fallback' ? 'Leitura de contingÃªncia' : 'Leitura pendente'}</span>
            <button type="button" className="nexus-topbar-analyst" onClick={onOpenZen} style={{ marginRight: "10px", background: "rgba(255,255,255,0.1)", color: "#fff", border: "1px solid rgba(255,255,255,0.2)" }}><Activity size={13} /> ZEN MODE</button> <button type="button" className="nexus-topbar-analyst" onClick={onOpenAnalyst}><Search size={13} /> Analista</button>
